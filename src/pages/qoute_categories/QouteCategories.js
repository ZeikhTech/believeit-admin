...
const QouteCategories = (props) => {
  const [categories, setCategories] = useState([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    // $("#qoute_categories_table").DataTable({
    //   language: {
    //     paginate: {
    //       next:
  ...
  const deleteCategory = async (category) => {
    ...
    try {
      ...
      handleErrors("Failed to delete Quote Category");
    }
  };
  const renderCategories = () => {
    return (
      <div style={{ position: "relative", overflowX: "scroll" }}>
        <table className="table" id="qoute_categories_table">
          <thead>
            <tr>
              <th>Name</th>
              <th>Free</th>
                ...
                <tr key={cat._id}>
                  ...
                  <td>
                    <Link
                      ...
                      to={basePath + "/qoute_categories/" + cat._id}
                    >
        ...
        <Card className="shadow">
          <CardHeader>
            <div className="clearfix">
              <h2 className="mb-0 float-left">Qoute Categories</h2>
              <Link
                to={basePath + "/qoute_categories/add"}
...
export default QouteCategories;
