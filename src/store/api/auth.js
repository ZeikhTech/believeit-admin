...
export const signinUser = ({ body = {}, onSuccess, onError, onEnd }) => {
  return async (dispatch, getState) => {
    try {
      ...
      const { user, token: xAuthToken } = res.data;

      if (user.role !== "ADMIN")
        return handleErrors("You are restricted in this area");

      http.setCommonHeaders("x-auth-token", xAuthToken);
...
export const signoutUser = ({ body = {}, onSuccess, onError, onEnd } = {}) => {
  return async (dispatch, getState) => {
    try {
      ...
      //   },
      // });
      http.setCommonHeaders("x-auth-token", "");
